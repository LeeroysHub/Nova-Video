...
public class MultipleSelectionManager implements ActionMode.Callback {
    ...
    private final AbsListView mLeeroyFlixGridView;
    ...
    public boolean onPrepareActionMode(ActionMode mode, Menu menu) {
        menu.findItem(R.string.delete).setVisible(mLeeroyFlixGridView.getCheckedItemCount() > 0);
        ...
        if(mLeeroyFlixGridView instanceof HeaderGridView){
            firstPosition += ((HeaderGridView) mLeeroyFlixGridView).getOffset();
        }
        else if(mLeeroyFlixGridView instanceof ListView){
            firstPosition += ((ListView) mLeeroyFlixGridView).getHeaderViewsCount()   ;
        }
        if(mLeeroyFlixGridView.getCheckedItemCount()>0) {
            for (int i = firstPosition; i < mLeeroyFlixGridView.getCount(); i++) {

                if (mLeeroyFlixGridView.getCheckedItemPositions().get(i)) {
        ...
        menu.findItem(R.string.video_browser_unindex_file).setVisible(areAllIndexed&&mLeeroyFlixGridView.getCheckedItemCount() > 0);
    ...
    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
        ...
        switch (item.getItemId()){
            case R.string.delete:
                ...
                for (int i = 0; i < mLeeroyFlixGridView.getCount(); i++) {
                    if (mLeeroyFlixGridView.getCheckedItemPositions().get(i)) {
