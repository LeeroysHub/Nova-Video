...
import org.leeroy.mediaplayer.cover.*;
...
public abstract class VideoSingleCursorCoverProvider extends SingleCursorCoverProvider {
	...
	protected Collection<Cover> convertCursorToCovers(Cursor c, boolean update) {
		...
		final int mScraperTypeIdx = c.getColumnIndexOrThrow(VideoStore.Video.VideoColumns.LEEROYFLIX_MEDIA_SCRAPER_TYPE);
		final int mScraperIdIdx = c.getColumnIndexOrThrow(VideoStore.Video.VideoColumns.LEEROYFLIX_MEDIA_SCRAPER_ID);
		...
		while (!c.isAfterLast()) {
			...
			if (update) {
				...
				if (scraperID > 0) {
					...
					} else if (scraperType == org.leeroy.mediascraper.BaseTags.TV_SHOW) {
