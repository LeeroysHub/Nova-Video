...
public class Paste extends AlertDialog implements FileManagerService.ServiceListener {
    ...
    public Paste(Context context) {
        ...
        View view = inflater.inflate(org.leeroy.filecorelibrary.R.layout.paste, null, false);
        setView(view);
        mOpenFile = (CheckBox)view.findViewById(org.leeroy.filecorelibrary.R.id.open_file);
        mOpenFile.setVisibility(View.GONE);
        mMessage = (TextView) view.findViewById(org.leeroy.filecorelibrary.R.id.message);
        mMessage.setMovementMethod(new ScrollingMovementMethod());

        mProgress = (ProgressBar) view.findViewById(org.leeroy.filecorelibrary.R.id.progress);
        mProgress.setMax(MAX_PROGRESS);
        mProgress.setEnabled(false);
        //mIndeterminateProgress = (ProgressBar) view.findViewById(R.id.progress_small);
        mProgressText = (TextView) view.findViewById(org.leeroy.filecorelibrary.R.id.progress_text);
            ...
            if( service.getPasteTotalProgress() != service.getPasteTotalSize()){
                if(service.getFilesToPaste().size()>0) {
                    ...
                    if (service.getFilesToPaste().size() > 1) {
                        mProgressText.setText(mContext.getResources().getString(org.leeroy.filecorelibrary.R.string.pasting_copy_many,
                        ...
                        mProgressText.setText(mContext.getResources().getString(org.leeroy.filecorelibrary.R.string.pasting_copy_one,
                                Formatter.formatShortFileSize(mContext, service.getPasteTotalProgress()), Formatter.formatShortFileSize(mContext, service.getPasteTotalSize())));
                    }
                }
            }
            else
                mProgressText.setText(mContext.getResources().getString(org.leeroy.filecorelibrary.R.string.pasting_done));
