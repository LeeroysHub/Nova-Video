...
public abstract class Browser extends Fragment implements AbsListView.OnScrollListener,
    ...
    protected AbsListView  mLeeroyFlixGridView;
    ...
    public void onResume() {
        ...
        if (mThumbnailRequester != null)
            mThumbnailRequester.refresh(mLeeroyFlixGridView);
        super.onResume();
    }
    ...
    public void onPause() {
        ...
        //Posted in mLeeroyFlixGridView to retrieve these values when the GridView is rendered.
        //risk of null values otherwise
        mLeeroyFlixGridView.post(new Runnable() {
            @Override
            public void run() {
                setPosition();
            }
        });
        mListState = mLeeroyFlixGridView.onSaveInstanceState();
    ...
    public void onDestroy() {
        ...
        if (mLeeroyFlixGridView != null)
            unregisterForContextMenu(mLeeroyFlixGridView);
    ...
    public void onSaveInstanceState(Bundle state) {
        ...
        if (mLeeroyFlixGridView != null) {
            state.putInt(FIRST_VISIBLE_POSITION, mLeeroyFlixGridView.getFirstVisiblePosition());
            state.putInt(SELECTED_POSITION, mLeeroyFlixGridView.getSelectedItemPosition());
            View v = mLeeroyFlixGridView.getChildAt(mSelectedPosition-mFirstVisiblePosition);
    ...
    public void onThumbnailReady(ThumbnailRequest request, ThumbnailEngine.Result result) {
        ...
        if(mLeeroyFlixGridView instanceof HeaderGridView){
            requestListPosition += ((HeaderGridView) mLeeroyFlixGridView).getOffset();
        }
        else if(mLeeroyFlixGridView instanceof ListView){
            requestListPosition += ((ListView) mLeeroyFlixGridView).getHeaderViewsCount()   ;
        }
        final int firstVisible = mLeeroyFlixGridView.getFirstVisiblePosition();
        final int lastVisible = mLeeroyFlixGridView.getLastVisiblePosition();
        ...
        if (firstVisible <= requestListPosition && requestListPosition <= lastVisible
            ...
            View itemView = mLeeroyFlixGridView.getChildAt(requestListPosition - firstVisible);
            if (itemView != null) {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                ...
                if (event.getAction() == KeyEvent.ACTION_DOWN && mLeeroyFlixGridView instanceof ListView) {
                    if (keyCode == KeyEvent.KEYCODE_DPAD_UP && mLeeroyFlixGridView.getFirstVisiblePosition() == mLeeroyFlixGridView.getSelectedItemPosition() && mLeeroyFlixGridView.getSelectedItemPosition() > 0) {
                        ...
                        mLeeroyFlixGridView.setSelection(mLeeroyFlixGridView.getSelectedItemPosition() - 1);
                        return true;


                    }
                    if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN && mLeeroyFlixGridView.getLastVisiblePosition() == mLeeroyFlixGridView.getSelectedItemPosition()
                            && mLeeroyFlixGridView.getSelectedItemPosition() < mLeeroyFlixGridView.getCount()) {
                        View v2 = mLeeroyFlixGridView.getChildAt(mLeeroyFlixGridView.getSelectedItemPosition() - mLeeroyFlixGridView.getFirstVisiblePosition());
                        ...
                        if (mLeeroyFlixGridView instanceof ListView)
                            ((ListView) mLeeroyFlixGridView).setSelectionFromTop(mLeeroyFlixGridView.getSelectedItemPosition() + 1,
                                    scrollTo);
                        return true;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if(mViewMode== VideoUtils.VIEW_MODE_GRID){
            mLeeroyFlixGridView = (AbsListView) mRootView.findViewById(R.id.leeroyflix_grid_view);
            mRootView.findViewById(R.id.leeroyflix_list_view).setVisibility(View.GONE);
        }
        else
        {
            mLeeroyFlixGridView = (AbsListView) mRootView.findViewById(R.id.leeroyflix_list_view);
            mRootView.findViewById(R.id.leeroyflix_grid_view).setVisibility(View.GONE);
        }
        initList((AbsListView) mRootView.findViewById(R.id.leeroyflix_list_view));
        initList((AbsListView)mRootView.findViewById(R.id.leeroyflix_grid_view));
    ...
    protected void setViewMode(int mode) {
        ...
        switch (mode) {
            ...
            default:
                mLeeroyFlixGridView = (AbsListView) mRootView.findViewById(R.id.leeroyflix_list_view);
                mRootView.findViewById(R.id.leeroyflix_grid_view).setVisibility(View.GONE);
                mLeeroyFlixGridView.setVisibility(View.VISIBLE);
                if(mLeeroyFlixGridView instanceof GridView)
                    ((GridView)mLeeroyFlixGridView).setNumColumns(1);
            ...
            case VideoUtils.VIEW_MODE_GRID:
                ...
                mLeeroyFlixGridView.setVisibility(View.VISIBLE);
                if(mLeeroyFlixGridView instanceof GridView)
                    ((GridView)mLeeroyFlixGridView).setNumColumns(GridView.AUTO_FIT);
        ...
        if(mLeeroyFlixGridView instanceof GridView){
            ((GridView)mLeeroyFlixGridView).setVerticalSpacing(verticalSpacing);
            ((GridView)mLeeroyFlixGridView).setStretchMode(stretchMode);
        }
    ...
    public final void bindAdapter() {
        ...
        if (mLeeroyFlixGridView.getAdapter() != mBrowserAdapter)
            mLeeroyFlixGridView.setAdapter(mBrowserAdapter);
        if (newAdapter) {
            setupThumbnail();
        } else {
    ...
    public void loading(){
        ...
        if(mLeeroyFlixGridView!=null && mRootView!=null){
            mLeeroyFlixGridView.setVisibility(View.GONE);
    ...
    protected void postBindAdapter() {
        ...
        if (mBrowserAdapter.isEmpty()) {
            ...
            /*mSelectedPosition = Utils.restoreBestPositionWithScroll(mLeeroyFlixGridView,
                    mSelectedPosition,
                    mFirstVisiblePosition,
                    mScroll);*/
            if(mListState!=null)
                mLeeroyFlixGridView.onRestoreInstanceState(mListState);
    ...
    public void setItemChecked(int position){
        ...
        mLeeroyFlixGridView.setItemChecked(position, !mLeeroyFlixGridView.isItemChecked(position));
    ...
    public void enableMultiple(int position, boolean toggle){
        if(mActionModeManager==null)
            mActionModeManager = new MultipleSelectionManager(this, mLeeroyFlixGridView, mBrowserAdapter);
        ...
        mLeeroyFlixGridView.setChoiceMode(AbsListView.CHOICE_MODE_MULTIPLE);
        if(toggle)
    ...
    public void disableMultiple(){
        ...
        mLeeroyFlixGridView.requestLayout();
        mLeeroyFlixGridView.post(new Runnable() {
            @Override
            public void run() {
                //needed because of a bug when setting choice mode before choices are cleared
                mLeeroyFlixGridView.setChoiceMode(AbsListView.CHOICE_MODE_NONE);
                mLeeroyFlixGridView.invalidateViews();
            }
        });
        getActivity().invalidateOptionsMenu();
    }
    ...
    protected void setPosition() {
        mSelectedPosition = mLeeroyFlixGridView.getSelectedItemPosition()>=0?mLeeroyFlixGridView.getSelectedItemPosition():mLeeroyFlixGridView.getFirstVisiblePosition();
        mFirstVisiblePosition = mLeeroyFlixGridView.getFirstVisiblePosition();
        View v = mLeeroyFlixGridView.getChildAt(mSelectedPosition-mFirstVisiblePosition);
        mScroll = (v == null) ? 0 : v.getTop();
    ...
    public void startDeletingDialog(List<Uri> uriToDelete){
        ...
        mLeeroyFlixGridView.getCheckedItemPosition();
