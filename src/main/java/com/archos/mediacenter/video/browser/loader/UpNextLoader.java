...
public class UpNextLoader extends VideoLoader {
    ...
    public String getSelection() {
        ...
        builder.append(
                ...
                "(s_id IS NOT NULL AND " + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 0 AND lfx_hiddenbyuser = 0 AND (bookmark IS NULL OR bookmark <= 0) AND " +
                "_id IN (SELECT _id FROM (SELECT v._id, ROW_NUMBER() OVER(PARTITION BY v.s_id ORDER BY v.e_season, v.e_episode) as rn FROM video v JOIN ( SELECT s_id, MAX(e_season * 1000 + e_episode) AS max_watched_episode FROM video WHERE " + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 1 AND lfx_hiddenbyuser = 0 GROUP BY s_id ) AS lw ON v.s_id = lw.s_id WHERE v." + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 0 AND v.lfx_hiddenbyuser = 0 AND (v.e_season * 1000 + v.e_episode) > lw.max_watched_episode ) WHERE rn = 1))" +
                " OR " +
                // Part 2: Next movies in collections (excluding in-progress)
                "(m_coll_id IS NOT NULL AND " + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 0 AND lfx_hiddenbyuser = 0 AND (bookmark IS NULL OR bookmark <= 0) AND " +
                "_id IN (SELECT _id FROM (SELECT v._id, ROW_NUMBER() OVER(PARTITION BY v.m_coll_id ORDER BY v.m_year) as rn FROM video v JOIN ( SELECT m_coll_id, MAX(m_year) AS max_watched_year FROM video WHERE " + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 1 AND lfx_hiddenbyuser = 0 GROUP BY m_coll_id ) AS lw ON v.m_coll_id = lw.m_coll_id WHERE v." + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 0 AND v.lfx_hiddenbyuser = 0 AND v.m_year > lw.max_watched_year ) WHERE rn = 1))" +
                ")"
        );
    ...
    public String getSortOrder() {
        ...
        return "COALESCE(" +
                ...
                "(SELECT lw.lfx_lastTimePlayed FROM video lw WHERE lw.s_id = video.s_id AND lw." + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 1 ORDER BY lw.e_season DESC, lw.e_episode DESC LIMIT 1), " +
                // For collections: get the play time of the most recently *watched* movie
                "(SELECT lw.lfx_lastTimePlayed FROM video lw WHERE lw.m_coll_id = video.m_coll_id AND lw." + VideoStore.Video.VideoColumns.ARCHOS_TRAKT_SEEN + " = 1 ORDER BY lw.m_year DESC LIMIT 1), " +
